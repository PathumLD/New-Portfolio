...
const LessonCard: React.FC<{ lesson: Lesson }> = ({ lesson }) => (
  <div className="bg-white/30 dark:bg-dark-background/30 backdrop-blur-lg rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 group border border-black/5 dark:border-white/10 hover:border-black/10 dark:hover:border-white/20 transform hover:-translate-y-1 flex flex-col h-full">
    ...
    <div className="p-6 flex-grow flex flex-col">
      <h3 className="text-xl font-bold mb-2 text-[#1c1a1c] dark:text-white">{lesson.title}</h3>
      <p className="text-[#1c1a1c]/70 dark:text-white/70 mb-4 text-sm flex-grow">{lesson.description}</p>
...
const GradeCard: React.FC<{ grade: Grade }> = ({ grade }) => (
    <Link to={`/tutoring/${grade.id}`} className="h-full bg-white/30 dark:bg-dark-background/30 backdrop-blur-lg rounded-2xl p-8 shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group border border-black/5 dark:border-white/10 relative overflow-hidden flex flex-col">
        ...
        <div className="relative flex flex-col flex-grow">
          <grade.icon className="h-12 w-12 text-green-500 mb-4 transition-colors group-hover:text-green-600" />
...
const TutoringPage: React.FC = () => {
    ...
    return (
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold tracking-tight text-[#1c1a1c] dark:text-white sm:text-5xl">{selectedGrade.level}</h1>
          <p className="mt-4 text-xl text-[#1c1a1c]/70 dark:text-white/70">Explore the lessons available for this grade.</p>
        </div>
  ...
  return (
    <div>
      <div className="text-center mb-12">
        <h1 className="text-4xl font-bold tracking-tight text-[#1c1a1c] dark:text-white sm:text-5xl">Tutoring Programs</h1>
        <p className="mt-4 text-xl text-[#1c1a1c]/70 dark:text-white/70">Select a grade level to see available lessons.</p>
      </div>
